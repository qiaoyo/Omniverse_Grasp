...
def get_scene_grasp(mesh_list,grasp_npy,camera_pos,camera_rot_im,camera_rot_real,part_pos,part_rot_im,part_rot_real):
    ...
    camera_rot[0]=camera_rot_im[random_index][camera_index]
    camera_rot[1:]=camera_rot_real[random_index][camera_index]
    ...
    for i,mesh in enumerate(mesh_list):
        if mesh=='001_1':
            print(i)
            mesh_rot=np.empty([4])
            mesh_rot[0]=part_rot_im[i]
            mesh_rot[1:]=part_rot_real[i]
...
def show_pc_scene(mesh_list,hand_point,part_pos,part_rot_im,part_rot_real,camera2world_T):
    for idx,mesh in enumerate(mesh_list):
        if mesh=='001_1':
            ...
            mesh_rot = np.empty([4])
            mesh_rot[0] = part_rot_im[idx]
            mesh_rot[1:] = part_rot_real[idx]
...
if __name__=='__main__':
    ...
    scene_path='/media/pika/Joyoyo/Omniverse_New/001_1/'

    grasp_npy_name='default_'+'001_1'+'_140.npy'
    ...
    camera2world_T = world2camera_T.inverse().matrix
    print('world2camera trans:', world2camera_trans)

    pcd=o3d.io.read_point_cloud('/home/pika/Desktop/0001.ply')
    pcd=o3d.io.read_point_cloud('/media/pika/Joyoyo/0808_backup/Desktop/0001.ply')
    ...
    camera2mesh_rot = camera2mesh[0:3, 0:3]
    camera2mesh_trans = camera2mesh[0:3, 3].reshape(-1, 3)
    print('world2camera trans:', world2camera_T)
    print('camera2world:',camera2world_T)
    print(pc.shape,camera2mesh_rot.shape,camera2mesh_trans.shape)
    pc = camera2mesh_rot.dot(pc).T + camera2mesh_trans

    center=camera2world_T[0:3,0:3].dot(part_pos[idx].T).T+camera2world_T[0:3,3]
    ...
    o3d.visualization.draw_geometries([pcd]+[pcd_hand_points]+[axis_pcd]+[raw_scene_pc]+[mesh_pcd])
