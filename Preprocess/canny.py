...
def do_job(i):  # 处理函数  处理第i个模型
    ...
    good_grasp_file_name = os.environ['HOME'] + "/good_grasp/{}_{}_{}".format(filename_prefix, str(object_name),
                                                                              str(len(good_grasp)))
...
def worker(i, sample_nums, grasp_amount, good_grasp):  # 主要是抓取采样器以及打分    100  20
    ...
    # 截取目标对象名称
    object_name = file_list_all[i][len(home_dir) + 22:]
    ...
    print("Log: do job", i)
    # 设置obj模型文件与sdf文件路径
    if os.path.exists(str(file_list_all[i]) + "/" + str(file_list_all[i])[-5:] + ".obj") and os.path.exists(
            str(file_list_all[i]) + "/" + str(file_list_all[i])[-5:] + ".sdf"):
        of = ObjFile(str(file_list_all[i]) + "/" + str(file_list_all[i])[-5:] + ".obj")
        sf = SdfFile(str(file_list_all[i]) + "/" + str(file_list_all[i])[-5:] + ".sdf")
    else:
    ...
    print('Gripper:{} Object:{} Rate:{:.4f} {}/{}'.
          format(gripper_name, object_name, good_grasp_rate, len(good_grasp), count))
...
if __name__ == '__main__':
    ...
    # 存放CAD模型的文件夹
    file_dir = home_dir + "/assemble_scale_grasp"  # 获取模型的路径
