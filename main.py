...
def create_component(Config_yaml,total_parts):
    ...
    for part in total_parts:
        ...
        print(part_name,position,orientation,quaternion2euler(orientation[[1,2,3,0]]))
        position[2]=i*10
        i=i+1
        ...
        quan=euler2quaternion(rotation)
        print("get:",part,rotation,quan[[3,0,1,2]])
...
def random_camera_pose(Config_yaml):
    ...
    for i in range(render_steps):
        ...
        # position_tuple=(float(0),float(0),float(position[i][2]))
...
def random_six_dof(Config_yaml):
    ...
    return position,orientation[[3,0,1,2]]
def save_6dof(Config_yaml,args,total_parts):
    ...
    for part in total_parts:
        ...
        scale, rotation, rotation_order, translation=omni.usd.utils.get_local_transform_SRT(part_prim)
        rot.append(rotation)
        pos.append(translation)
        quan=euler2quaternion(rotation)
        rot_im.append(quan[0:3])
        rot_rel.append([quan[3]])
        print(part,rotation,quan)
    f.close()
...
def get_camera_6dof(Config_yaml):
    ...
    for i in range(Camera_num):
        ...
        scale, rotation, rotation_order, translation=omni.usd.utils.get_local_transform_SRT(camera_prim)
        camera_pos.append(translation)
        quan=euler2quaternion(rotation)
        camera_rot_im.append(quan[0:3])
        camera_rot_rel.append(quan[3])
        print(i,translation,quan,rotation)
...
if __name__=='__main__':
        ...
        for i in range(camera_num):
            ...
            with rep_camera_list[i]:
                rep.modify.pose(
                    ...
                    look_at= [0,0,0]#rep.distribution.sequence(camera_look_at_prim_list),
                )
    # The Render Process
    step=0
    ...
    # np.save(output_directory+'Camera_Rot.npy',np.array(saved_camera_rot))
